...
class Database:
    ...
    def init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            conn.commit()
            print("[DB] База данных создана успешно!")
        ...
        with self.lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO accounts (roblox_login, roblox_pass, email)
                        VALUES (?, ?, ?)
                    """, (roblox_login, roblox_pass, email))
                    conn.commit()
                    return True
            except sqlite3.IntegrityError as e:
                # Проверяем, что именно дублируется
                if "email" in str(e):
                    print(f"[DB] ⚠️ Email {email} уже существует в базе")
                return False
    ...
    def mark_account_sold(self, account_id, buyer_id, funpay_order_id, funpay_chat_id):
        with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    UPDATE accounts
                    SET status = 'sold', sold_to = ?, sold_at = ?, funpay_order_id = ?, funpay_chat_id = ?
                    WHERE id = ?
    ...
    def mark_account_transferred(self, account_id):
        with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("UPDATE accounts SET status = 'transferred' WHERE id = ?", (account_id,))
                conn.commit()
    ...
    def create_order(self, funpay_order_id, buyer_id, buyer_name, account_id):
        with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT OR IGNORE INTO orders (funpay_order_id, buyer_id, buyer_name, account_id)
                    VALUES (?, ?, ?, ?)
    ...
    def update_order_status(self, funpay_order_id, status):
        with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if status == 'completed':
                    cursor.execute("""
                        UPDATE orders SET status = ?, completed_at = ? WHERE funpay_order_id = ?
                    """, (status, datetime.now(), funpay_order_id))
                else:
                    cursor.execute("UPDATE orders SET status = ? WHERE funpay_order_id = ?", (status, funpay_order_id))
                conn.commit()
