...
class EmailChecker:
    ...
    def find_roblox_code(self, target_email, timeout=120, check_interval=5):
        """Ищет код верификации от Roblox в Gmail"""
        if not self.imap:
            if not self.connect():
                return None
        ...
        while time.time() - start_time < timeout:
            try:
                # Проверяем состояние IMAP
                if self.imap.state not in ['AUTH', 'SELECTED']:
                    logger.warning(f"[EMAIL] IMAP state: {self.imap.state}, переподключаюсь")
                    self.disconnect()
                    if not self.connect():
                ...
                if messages[0]:
                    ...
                    # Берём последние 5 писем (на случай если их несколько)
                    for latest_id in reversed(msg_ids[-5:]):
                        ...
                        if code_match:
                            ...
                            logger.info(f"[EMAIL] ✅ Найден код {code} для {target_email}")
                            return code

                        alt_match = re.search(
                            r'(?:code is|код|verification code|security code)[^\d]*(\d{6})',
                            body, re.IGNORECASE
                        )
                        if alt_match:
        ...
        logger.warning(f"[EMAIL] ❌ Код не найден за {timeout} сек для {target_email}")
        return None
